...
export class ParameterApi {
     ...
     */
    public async updateParameters (projectId: string, connectionId: number, ideaParameterUpdate?: Array<IdeaParameterUpdate>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<ParameterData>;  }> {
        const localVarPath = this.basePath + '/api/1/projects/{projectId}/connections/{connectionId}/parameters'
            .replace('{' + 'projectId' + '}', encodeURIComponent(String(projectId)))
            .replace('{' + 'connectionId' + '}', encodeURIComponent(String(connectionId)));
        let localVarQueryParameters: any = {};
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Array<ParameterData>;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "Array<ParameterData>");
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
                        }
