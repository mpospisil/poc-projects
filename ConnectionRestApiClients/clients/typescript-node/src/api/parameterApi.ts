...
export class ParameterApi {
     ...
     */
    public async updateParameters (projectId: string, connectionId: number, ideaParameterUpdate?: Array<IdeaParameterUpdate>, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<IdeaParameter>;  }> {
        const localVarPath = this.basePath + '/api/1/projects/{projectId}/connections/{connectionId}/parameters'
            .replace('{' + 'projectId' + '}', encodeURIComponent(String(projectId)))
            .replace('{' + 'connectionId' + '}', encodeURIComponent(String(connectionId)));
        let localVarQueryParameters: any = {};
